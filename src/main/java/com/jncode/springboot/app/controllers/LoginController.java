...
public class LoginController {
	...
	public String login(@RequestParam(value = "error", required = false) String error,@RequestParam(value = "logout",required = false) String logout , Model model, Principal principal, RedirectAttributes flash) {
		...
		if(principal!=null) {
			...
			flash.addFlashAttribute("info", "Ya estas autenticado");
			
			return "redirect:/";
		}
		
		if(error!=null) {
			model.addAttribute("error", "Nombre de usuario o contraseña incorrectos, verifique sus datos e intente nuevamente");
		}
